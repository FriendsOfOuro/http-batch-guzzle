...
final class RecordingHttpClientTest extends TestCase
{
    ...
    public function test_it_records_successful_batch_request_and_response(): void
    {
        $request2 = $this->createMock(RequestInterface::class);
        $response2 = $this->createMock(ResponseInterface::class);
        $requests = [$this->request, $request2];

        $batchItems = [
            new BatchItem($this->request, $this->response),
            new BatchItem($request2, $response2),
        ];
        $batch = new ResponseBatch($batchItems);
        ...
        $this->assertSame([$requests], $this->recordingClient->getRecordedBatchRequests());
        $this->assertSame([$batch], $this->recordingClient->getRecordedBatchResponses());
        $this->assertEmpty($this->recordingClient->getRecordedBatchExceptions());
        $this->assertSame($requests, $this->recordingClient->getLastBatchRequest());
        $this->assertSame($batch, $this->recordingClient->getLastBatchResponse());
        $this->assertNull($this->recordingClient->getLastBatchException());
        $this->assertEquals(1, $this->recordingClient->getBatchRequestCount());
        $this->assertEquals(1, $this->recordingClient->getBatchResponseCount());
        $this->assertEquals(0, $this->recordingClient->getBatchExceptionCount());
        $this->assertFalse($this->recordingClient->hasBatchExceptions());

        // Check that individual requests/responses from batch are also recorded
        $this->assertSame([$this->request, $request2], $this->recordingClient->getRecordedRequests());
        $this->assertSame([$this->response, $response2], $this->recordingClient->getRecordedResponses());
        $this->assertEquals(2, $this->recordingClient->getRequestCount());
        $this->assertEquals(2, $this->recordingClient->getResponseCount());
    }

    public function test_it_records_batch_request_and_exception(): void
    {
        $request2 = $this->createMock(RequestInterface::class);
        $requests = [$this->request, $request2];
        $exception = $this->createMock(ClientExceptionInterface::class);
        ...
        try {
            $this->recordingClient->sendRequestBatch($requests);
        } finally {
            $this->assertSame([$requests], $this->recordingClient->getRecordedBatchRequests());
            $this->assertEmpty($this->recordingClient->getRecordedBatchResponses());
            $this->assertSame([$exception], $this->recordingClient->getRecordedBatchExceptions());
            $this->assertSame($requests, $this->recordingClient->getLastBatchRequest());
            $this->assertNull($this->recordingClient->getLastBatchResponse());
            $this->assertSame($exception, $this->recordingClient->getLastBatchException());
            $this->assertEquals(1, $this->recordingClient->getBatchRequestCount());
            $this->assertEquals(0, $this->recordingClient->getBatchResponseCount());
            $this->assertEquals(1, $this->recordingClient->getBatchExceptionCount());
            $this->assertTrue($this->recordingClient->hasBatchExceptions());
        }
    }

    public function test_it_records_mixed_batch_with_successes_and_failures(): void
    {
        $request2 = $this->createMock(RequestInterface::class);
        $response2 = $this->createMock(ResponseInterface::class);
        $exception = $this->createMock(ClientExceptionInterface::class);
        ...
        $batchItems = [
            new BatchItem($this->request, $this->response), // Success
            new BatchItem($request2, null, $exception),      // Failure
        ];
        $batch = new ResponseBatch($batchItems);
        ...
        // Check batch recording
        $this->assertEquals(1, $this->recordingClient->getBatchRequestCount());
        $this->assertEquals(1, $this->recordingClient->getBatchResponseCount());

        // Check individual request/response recording from batch
        $this->assertSame([$this->request, $request2], $this->recordingClient->getRecordedRequests());
        $this->assertSame([$this->response], $this->recordingClient->getRecordedResponses());
        $this->assertSame([$exception], $this->recordingClient->getRecordedExceptions());
        $this->assertEquals(2, $this->recordingClient->getRequestCount());
    ...
    public function test_it_filters_successful_and_failed_batch_requests(): void
    {
        $requests1 = [$this->request];
        $requests2 = [$this->createMock(RequestInterface::class)];

        $batch = new ResponseBatch([new BatchItem($this->request, $this->response)]);
        $exception = $this->createMock(ClientExceptionInterface::class);

        $this->innerClient
            ->expects($this->exactly(2))
            ->method('sendRequestBatch')
            ->willReturnCallback(function (array $requests) use ($requests1, $batch, $exception) {
                if ($requests === $requests1) {
                    return $batch;
                }
                throw $exception;
            })
        ...
        $this->recordingClient->sendRequestBatch($requests1);

        try {
            $this->recordingClient->sendRequestBatch($requests2);
        } catch (ClientExceptionInterface) {
            // Expected
        }

        $this->assertSame([$requests1], $this->recordingClient->getSuccessfulBatchRequests());
        $this->assertSame([$requests2], $this->recordingClient->getFailedBatchRequests());
    }
    ...
    public function test_empty_state_returns_null_and_zero_counts(): void
    {
        ...
        $this->assertNull($this->recordingClient->getLastBatchRequest());
        $this->assertNull($this->recordingClient->getLastBatchResponse());
        $this->assertNull($this->recordingClient->getLastBatchException());
        $this->assertNull($this->recordingClient->getLastBatchInteraction());
        $this->assertEquals(0, $this->recordingClient->getRequestCount());
        $this->assertEquals(0, $this->recordingClient->getResponseCount());
        $this->assertEquals(0, $this->recordingClient->getExceptionCount());
        $this->assertEquals(0, $this->recordingClient->getBatchRequestCount());
        $this->assertEquals(0, $this->recordingClient->getBatchResponseCount());
        $this->assertEquals(0, $this->recordingClient->getBatchExceptionCount());
        $this->assertFalse($this->recordingClient->hasExceptions());
        $this->assertFalse($this->recordingClient->hasBatchExceptions());
        ...
        $this->assertEmpty($this->recordingClient->getSuccessfulBatchRequests());
        $this->assertEmpty($this->recordingClient->getFailedBatchRequests());
    }
}